...
export function Page({ size = 'lg', gap = 'lg', className, children, ...rest }: PageProps) {
  return (
    <main className={cn('min-h-[60vh] bg-background py-5 mobile-portrait:py-7 desktop:py-8', className)} {...rest}>
...
export function StateHint({
  ...
  const tone =
    kind === 'error' ? 'text-danger' : kind === 'empty' ? 'text-muted-foreground' : 'text-muted-foreground'
  return (
    <p
