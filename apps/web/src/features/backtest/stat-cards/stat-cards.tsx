  ...
  return (
    <div className={statGrid}>
      ...
      <div className={statCard}>
        ...
        <div className={statValue}>{p && p.sharpe !== null ? p.sharpe.toFixed(2) : '—'}</div>
      </div>
      ...
      <div className={statCard}>
        <div className={statLabel}>最优超额胜率{excess ? ` (${excess.strategy})` : ''}</div>
        <div className={`${statValue} ${colorClass(excess?.value)}`}>{pp(excess?.value)}</div>
      </div>

      <div className={statCard}>
        <div className={statLabel}>最强选择性{selectivity ? ` (${selectivity.strategy})` : ''}</div>
        <div className={statValue}>{selectivity ? pct(selectivity.value) : '—'}</div>
      </div>
    </div>
  )
}
