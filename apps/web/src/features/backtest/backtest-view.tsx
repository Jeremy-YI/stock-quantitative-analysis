...
export default function BacktestView() {
  ...
  const effectiveHold =
    verification?.hold_days.includes(holdDays) ? holdDays : verification?.hold_days[0] ?? 1

  const histogramBins = verification
    ? aggregateHistogram(verification.by_strategy, effectiveHold, histStrategy)
    : []

  const decaySeries = verification?.decay.find(
    (d) => d.strategy === (decayStrategy ?? strategiesWithDecay[0])
  ) ?? null
                ...
                <div className="flex items-end gap-3">
                  ...
                  <div className={field}>
                    ...
                    <select
                      ...
                      value={histStrategy ?? ''}
                      onChange={(event) =>
                        setHistStrategy(event.target.value || null)
                      }
                      className={selectClass}
                    >
                      <option value="">全部策略</option>
