...
import { chartCard, chartContainer, legendHint } from '../style'
...
export default function IndicatorPanel({
  ...
  return (
    <Card className={chartCard}>
      ...
      <CardContent>
        {loading && (
          <Skeleton className='h-[280px] w-full mobile-portrait:h-[380px] desktop:h-[520px]' />
        )}
        {!loading && error && <p className='py-10 text-center text-down'>{error}</p>}
        {!loading && !error && (
