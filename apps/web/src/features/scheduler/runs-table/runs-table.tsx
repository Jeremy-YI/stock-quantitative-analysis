...
import { emptyHint, rowHover, statusClass, statusLabel, tableWrap, td, th } from '../style'
  ...
  const columns = useMemo<ColumnDef<Run>[]>(
    () => [
      ...
      },
      {
        ...
        header: '开始时间',
        cell: (info) => (
          <span className='text-muted-foreground'>{fmtDateTime(info.getValue<string>())}</span>
        ),
      },
      {
        id: 'duration',
